...
- `--verbose` 等のリクエストダンプを将来追加する場合も、`Authorization: Bearer ...` ヘッダはマスク
...
- ストリーム/一括の既定は **stdout が TTY ならストリーム、非 TTY（パイプ）なら一括**（UNIX 慣習）。`--no-stream` で明示制御も可
